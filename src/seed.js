...
async function seed() {
  ...
  const existing = await db.findStaffByUsername(username);

  if (existing) {
    console.log(`Staff user "${username}" already exists... skipping.`);
    console.log(
      "Delete data/app.db and re-run if you want to reset everything.",
    );
    return;
  }

  await db.addStaffUser({ username, passwordHash });
  console.log(
    `Staff user "${username}" created. You can now log in on /login.html`,
  );
}
